...
export function useUserProfile() {
  ...
  const factsLoadingMore = useUserProfileStore((s) => s.factsLoadingMore);
  const factsHasMore = useUserProfileStore((s) => s.factsHasMore);
  const fetchProfile = useUserProfileStore((s) => s.fetchProfile);
  const fetchUserFacts = useUserProfileStore((s) => s.fetchUserFacts);
  const loadMoreUserFacts = useUserProfileStore((s) => s.loadMoreUserFacts);
